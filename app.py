...
if user_menu=='Medal Tally':
    ...
    fig1=px.line(nations_over_time,x='Years',y='Number of Countries',width=850, height=500)
    st.plotly_chart(fig1)

    events_over_time = calculations.data_over_time(df, 'Event', 'Number of Events')
    st.markdown("***")
    st.title("Events Over The Years")
    fig2=px.line(events_over_time, x='Years', y='Number of Events',width=850, height=500)
    st.plotly_chart(fig2)

    events_over_time = calculations.data_over_time(df, 'Name', 'Number of Athletes')
    st.markdown("***")
    st.title("Athletes Over The Years")
    fig3=px.line(events_over_time, x='Years', y='Number of Athletes',width=850, height=500)
    st.plotly_chart(fig3)

    st.markdown("***")
    st.title("Number Of Events Over Time( Per Every Sport)")
    fig4 = px.imshow(calculations.reshape_for_heatmap(df), width=900, height=800)
    ...
    fig5=px.line(country_df,x='Year',y='Medal',width=850, height=500)
    st.title(f"{selected_country} Medal Tally Over The Years")
    st.plotly_chart(fig5)

    st.markdown("***")
    st.title(f"{selected_country} Excells In the following Sports")
    fig6 = px.imshow(calculations.reshape_for_countrywise_heatmap(df,selected_country), width=850, height=500)
    ...
    fig7.update_layout(autosize=False,width=850, height=500)
    ...
    fig8.update_layout(autosize=False,width=850, height=500)
    ...
    fig9.update_layout(autosize=False, width=850, height=500)
    st.plotly_chart(fig9)

    st.markdown("***")
    st.title("Men vs Women Participation")
    m_vs_w=calculations.men_vs_women(df)
    fig10=px.line(m_vs_w, 'Year', ['Male', 'Female'],width=850, height=500)
