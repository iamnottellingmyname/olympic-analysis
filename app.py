...
df=pd.read_csv(r'C:\Users\Anurag Dabas\Desktop\pysparks\olympics analysis\athlete_events.csv')
region_df=pd.read_csv(r'C:\Users\Anurag Dabas\Desktop\pysparks\olympics analysis\noc_regions.csv')
...
if user_menu=='Medal Tally':
    ...
    fig1=px.line(nations_over_time,x='Years',y='Number of Countries',width=950, height=600)
    st.plotly_chart(fig1)

    events_over_time = calculations.data_over_time(df, 'Event', 'Number of Events')
    st.markdown("***")
    st.title("Events Over The Years")
    fig2=px.line(events_over_time, x='Years', y='Number of Events',width=950, height=600)
    st.plotly_chart(fig2)

    events_over_time = calculations.data_over_time(df, 'Name', 'Number of Athletes')
    st.markdown("***")
    st.title("Athletes Over The Years")
    fig3=px.line(events_over_time, x='Years', y='Number of Athletes',width=950, height=600)
    st.plotly_chart(fig3)

    st.markdown("***")
    st.title("Number Of Events Over Time( Per Every Sport)")
    fig4 = px.imshow(calculations.reshape_for_heatmap(df), width=1000, height=950)
    ...
    fig5=px.line(country_df,x='Year',y='Medal',width=950, height=600)
    st.title(f"{selected_country} Medal Tally Over The Years")
    st.plotly_chart(fig5)

    st.markdown("***")
    st.title(f"{selected_country} Excells In the following Sports")
    fig6 = px.imshow(calculations.reshape_for_countrywise_heatmap(df,selected_country), width=1000, height=950)
    ...
    fig7.update_layout(autosize=False,width=950, height=600)
    ...
    fig8.update_layout(autosize=False,width=950, height=600)
    ...
    fig9.update_layout(autosize=False, width=950, height=600)
    st.plotly_chart(fig9)

    st.markdown("***")
    st.title("Men vs Women Participation")
    m_vs_w=calculations.men_vs_women(df)
    fig10=px.line(m_vs_w, 'Year', ['Male', 'Female'],width=950, height=600)
